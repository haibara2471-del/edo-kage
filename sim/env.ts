 ...
 * v0.54d 放大 K 0.1→0.2：目标就是鼓励吸血，reward 应比承伤强让模型主动低血开吸血。
 * 精算：K=0.2 时 hp90 回40→+9.6，hp20 回40→+20.8（低血价值 2倍+）。
 * 换血账：挨10点再吸7点 = -0.10×scale（微亏）；且吸血耗 20 气 + CD 480帧，不可持续刷。
 * K≥0.25 会真鼓励换血（+0.25×scale 净赚）——0.2 是"放大但安全"上限。
 * 满血开 buff 回血量=0 → 无收益。吸血需命中才回血 → 同时鼓励开了就输出。 */
const VAMP_REWARD = 0.2;
...
export class GameEnv {
  ...
  async step(actionIdx: number): Promise<{ obs: number[]; reward: number; done: boolean; info: Record<string, number> }> {
    ...
    // 预算：一次吸血回血 40 → +4（回血 reward）+ 启动 +3；wave2 净收入 ~230，开 1-2 次占 3-6%，略升但仍可控。
    const vampActivated =
      actionIdx === 10 && this.player.hp < this.player.maxHp && this.player.vampCd <= 0 && this.player.ki >= 20;
    const vampActivateReward = vampActivated ? VAMP_ACTIVATE_REWARD : 0;
    ...
    let reward =
      ...
      vampReward +         // 吸血回血（v0.52）：教低血线适时开吸血，增强长周期续航
      vampActivateReward;  // 吸血启动（v0.52b）：非满血按吸血键引导探索，打破冷启动
    let done = false;
  ...
  private observeFor(p: Player, enemies: Hittable[]): number[] {
    ...
    // 吸血状态（v0.52b）：CD 剩余比例 + buff 剩余比例。用尾部 2 个 padding 空位，42 维不变。
